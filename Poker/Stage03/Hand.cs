...
namespace StageThree
{
    public class Hand
    {
        ...
        /* Since a collection of pairs needs to be built, a mutable collection must be temporarily created.
         Because the scope is limited, risk for side-effect is minimal. 
         In addition, the ConcurrentDictionary's AddOrUpdate method is Thread Safe.
         While the ConcurrentDictionary mutates state via AddOrUpdate, 
         it does use a functional principal called a higher order function.
